...
pub mod route_1 {
    pub async fn entrypoint<'a>(s_0: &'a dep_daa9931d::A) -> pavex::response::Response {
        let response = wrapping_0(s_0).await;
        response
    }
    async fn stage_1<'a>(s_0: &'a dep_daa9931d::A) -> pavex::response::Response {
        let response = handler(s_0).await;
        response
    }
    async fn wrapping_0(v0: &dep_daa9931d::A) -> pavex::response::Response {
    ...
    async fn handler(v0: &dep_daa9931d::A) -> pavex::response::Response {
    ...
    where
        T: std::future::Future<Output = pavex::response::Response>,
    {
        s_0: &'a dep_daa9931d::A,
        next: fn(&'a dep_daa9931d::A) -> T,
    }
    impl<'a, T> std::future::IntoFuture for Next0<'a, T>
    where
        T: std::future::Future<Output = pavex::response::Response>,
