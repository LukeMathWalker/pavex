...
fn test_error_types() {
    ...
    // Test that we can create SqlxError variants
    let _error = SqlxError::RowNotFound;

    // Test that session IDs work correctly
    let another_id = SessionId::random();
    assert_ne!(session_id.inner(), another_id.inner());
}
