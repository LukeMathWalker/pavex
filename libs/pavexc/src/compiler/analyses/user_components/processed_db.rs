    ...
    fn invalid_config_type(
        ...
        let label = source.as_ref().and_then(|source| {
            ...
            match &e {
                ...
                KeyMustAValidRustIdentifier { .. } => {
                    diagnostic::get_config_key_span(source, location)
        ...
        let help: Option<String> = match &e {
            ConfigTypeValidationError::KeyMustAValidRustIdentifier { key, .. } => {
                // TODO: Explain _why_ the key is not a valid Rust identifier.
                writeln!(&mut error_msg, "\n`{key}` is not a valid Rust identifier.").unwrap();
                None
            }
            ConfigTypeValidationError::CannotHaveAnyLifetimeParameters { ty } => {
                ...
                if !elided.is_empty() {
                    let n = elided.len();
                    let part = if n == 1 {
                        format!("1 elided lifetime parameter")
                    } else {
                ...
                if !static_.is_empty() {
                    let n = static_.len();
                    let part = if n == 1 {
                        format!("1 static lifetime parameter")
                    } else {
