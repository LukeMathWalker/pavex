...
use crate::compiler::analyses::user_components::source::BlueprintSource;
...
fn process_route(
    ...
    let component = UserComponent::RequestHandler {
        ...
        source: BlueprintSource {
            identifiers_id: raw_callable_identifiers_id,
            scope_id: route_scope_id,
        },
    };
    let request_handler_id = intern_component(
        aux,
        component,
        ROUTE_LIFECYCLE,
        registered_route.request_handler.registered_at.to_owned(),
    );
...
fn process_fallback(
    ...
    let fallback_id = intern_component(
        aux,
        component,
        ROUTE_LIFECYCLE,
        fallback.request_handler.registered_at.to_owned(),
    );
...
fn process_middleware(
    ...
    let component_id = intern_component(
        aux,
        component,
        MIDDLEWARE_LIFECYCLE,
        middleware.middleware.registered_at.clone(),
    );
...
fn process_constructor(
    ...
    let constructor_id = intern_component(
        aux,
        component,
        lifecycle,
        constructor.constructor.registered_at.clone(),
    );
...
fn process_error_observer(
    ...
    let id = intern_component(
        aux,
        component,
        LIFECYCLE,
        eo.error_observer.registered_at.clone(),
    );
...
fn process_prebuilt_type(aux: &mut AuxiliaryData, si: &PrebuiltType, current_scope_id: ScopeId) {
    ...
    let id = intern_component(aux, component, LIFECYCLE, si.input.registered_at.clone());
    aux.id2cloning_strategy.insert(
        id,
...
fn process_config_type(aux: &mut AuxiliaryData, t: &ConfigType, current_scope_id: ScopeId) {
    ...
    let id = intern_component(aux, component, LIFECYCLE, t.input.registered_at.clone());
    aux.id2cloning_strategy.insert(
        id,
        t.cloning_strategy
...
fn intern_component(
    ...
    let component_id = aux.component_interner.get_or_intern(component);
    aux.id2lifecycle.insert(component_id, lifecycle);
    aux.id2locations.insert(component_id, location);
    component_id
}
...
fn process_error_handler(
    ...
    intern_component(
        ...
        component,
        lifecycle,
        error_handler.registered_at.to_owned(),
    );
}
...
mod diagnostics {
    ...
    use crate::{
        ...
        diagnostic::{self, OptionalLabeledSpanExt, OptionalSourceSpanExt},
    };
    ...
    pub(super) fn route_path_must_start_with_a_slash(
        ...
        let location = &aux.id2locations[&route_id];
        let source = diagnostics.source(location).map(|s| {
            diagnostic::route_path_span(s.source(), location)
