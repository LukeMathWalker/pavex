        ...
        let scope_based_fallback_tree = {
            ...
            let scope_id2fallback_id = {
                ...
                for (id, component) in aux.iter() {
                    let UserComponent::Fallback { source, .. } = component else {
                        continue;
                    };
                    let parents = source.scope_id.direct_parent_ids(scope_graph);
                    assert_eq!(
                        parents.len(),
                        1,
    ...
    fn either_all_domain_based_or_all_agnostic(
        ...
        let domain_based_snippet = {
            let id = aux
                ...
                .unwrap();
            let location = &aux.id2locations[&id];
...
impl PathRouter {
    fn new(
        ...
        let root_scope_id = scope_graph
            .find_common_ancestor(component_ids.iter().map(|id| aux[*id].scope_id()).collect());
    ...
    > {
        ...
        for id in component_ids.iter() {
            ...
            let scope_fallback_id =
                scope_based_fallback_router.find_fallback_id(source.scope_id, scope_graph);
            match path_fallback {
                None => {
...
fn push_fallback_ambiguity_diagnostic(
    ...
    let route_location = &db.id2locations[&route_id];
    let route_source = diagnostics.source(route_location).map(|s| {
        diagnostic::route_path_span(s.source(), route_location)
    ...
    let scope_fallback = {
        ...
        format!(
            "`{}`",
            db.identifiers_interner[source.identifiers_id]
...
fn push_fallback_method_ambiguity_diagnostic(
    ...
    for (i, (fallback_id, handler_id2methods)) in fallback_id2handler_id.iter().enumerate() {
        let fallback_path = {
            ...
            let location = &db.id2locations[fallback_id];
            let snippet = diagnostics.source(&location).map(|s| {
            ...
            let fallback_path = db.identifiers_interner[source.identifiers_id]
                .fully_qualified_path()
                .join("::");
...
fn push_router_conflict_diagnostic(
    ...
    for (i, id) in ids.iter().enumerate() {
        let location = &db.id2locations[*id];
        let s = diagnostics.source(&location).map(|s| {
            diagnostic::f_macro_span(s.source(), location)
                .labeled(format!("The {} conflicting handler", ZeroBasedOrdinal(i)))
                .attach(s)
        });
        builder = builder.optional_source(s);
    }
