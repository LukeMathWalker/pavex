...
use crate::{
    ...
    diagnostic::{
        AnnotatedSource, CallableDefinition, CompilerDiagnostic, OptionalSourceSpanExt,
        SourceSpanExt,
    },
};
...
fn invalid_config_type(
    ...
    let source = diagnostics.source(&location).map(|s| {
        ...
        match &e {
            ...
            InvalidKey { .. } => diagnostic::config_key_span(s.source(), location)
...
fn cannot_resolve_callable_path(
    ...
    let source = diagnostics.source(&location);
    match e {
