...
use crate::{
    compiler::component::DefaultStrategy,
...
impl ApplicationConfig {
    ...
    pub fn new(
        ...
        let mut key2types: BTreeMap<String, IndexMap<ResolvedType, ComponentId>> = BTreeMap::new();
        let mut type2keys: IndexMap<ResolvedType, BTreeMap<String, ComponentId>> = IndexMap::new();
        for (id, _) in component_db.iter() {
            ...
            let ident: syn::Ident = syn::parse_str(config.key())
                .expect("The config key has already been confirmed to be a valid Rust identifier");
...
fn same_type_different_key(
    ty: &ResolvedType,
    key2component_id: &BTreeMap<String, ComponentId>,
    component_db: &ComponentDb,
    package_graph: &PackageGraph,
    diagnostics: &mut Vec<miette::Error>,
) {
