...
pub mod route_0 {
    ...
    async fn post_processing_0(v0: pavex::Response) -> pavex::Response {
        let v1 = post::root_span::root_span();
        let v2 = post::logger::response_logger(v0, &v1);
        <pavex::Response as pavex::IntoResponse>::into_response(v2)
    }
    async fn post_processing_1(v0: pavex::Response) -> pavex::Response {
        let v1 = post::compress::compress(v0);
        let v2 = match v1 {
            Ok(ok) => ok,
            Err(v2) => {
                return {
                    let v3 = post::errors::CompressionError::to_response(&v2);
                    <pavex::Response as pavex::IntoResponse>::into_response(v3)
                };
            }
        };
