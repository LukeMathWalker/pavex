...
impl Router {
    ...
    fn router() -> matchit::Router<u32> {
        ...
        router.insert("/order1/", 1u32).unwrap();
        router.insert("/order2/", 2u32).unwrap();
        router.insert("/post_and_wrap/", 3u32).unwrap();
        router.insert("/post_only/", 4u32).unwrap();
        router.insert("/pre_and_post/", 5u32).unwrap();
        router.insert("/pre_and_wrap/", 6u32).unwrap();
        router.insert("/pre_only/", 7u32).unwrap();
        router.insert("/wrap_only/", 8u32).unwrap();
        router
    }
    pub async fn route(
        &self,
...
pub mod route_4 {
    ...
    async fn post_processing_0(v0: pavex::Response) -> pavex::Response {
        let v1 = order::post2(v0);
        <pavex::Response as pavex::IntoResponse>::into_response(v1)
    }
    async fn post_processing_1(v0: pavex::Response) -> pavex::Response {
        let v1 = order::post1(v0);
        <pavex::Response as pavex::IntoResponse>::into_response(v1)
    }
    struct Next0<T>
    where
