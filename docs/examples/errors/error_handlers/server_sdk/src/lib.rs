...
impl Router {
    ...
    fn router() -> matchit::Router<u32> {
        ...
        router.insert("/bearer", 0u32).unwrap();
    ...
    pub async fn route(
        ...
        match matched_route.value {
            0u32 => match &request_head.method {
                &pavex::http::Method::POST => route_1::entrypoint(&request_head).await,
                _ => {
                    let allowed_methods: pavex::router::AllowedMethods =
            ...
            1u32 => match &request_head.method {
                &pavex::http::Method::POST => route_2::entrypoint(&request_head).await,
                _ => {
                    let allowed_methods: pavex::router::AllowedMethods =
                        pavex::router::MethodAllowList::from_iter([pavex::http::Method::POST])
                            .into();
...
pub mod route_1 {
    ...
    async fn handler(v0: &pavex::request::RequestHead) -> pavex::Response {
        let v1 = error_handlers::bearer_token::BearerToken::extract(v0);
        let v2 = match v1 {
            Ok(ok) => ok,
            Err(v2) => {
                return {
                    let v3 = error_handlers::bearer_token::BearerExtractionError::to_response(&v2);
                    <pavex::Response as pavex::IntoResponse>::into_response(v3)
                };
            }
        };
        let v3 = error_handlers::bearer_token::auth(&v2);
        <pavex::Response as pavex::IntoResponse>::into_response(v3)
    }
    struct Next0<'a, T>
    where
...
pub mod route_2 {
    ...
    async fn handler(v0: &pavex::request::RequestHead) -> pavex::Response {
        let v1 = error_handlers::login::login(v0);
        let v2 = match v1 {
            Ok(ok) => ok,
            Err(v2) => {
                return {
                    let v3 = error_handlers::login::LoginError::to_response(&v2);
