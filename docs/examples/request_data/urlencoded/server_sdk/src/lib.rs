...
impl Router {
    ...
    fn router() -> matchit::Router<u32> {
        ...
        router.insert("/search", 0u32).unwrap();
        router
    }
    pub async fn route(
        &self,
...
pub mod route_1 {
    ...
    async fn handler(
        ...
        let v4 = match v3 {
            ...
            Err(v4) => {
                return {
                    let v5 =
                        pavex::request::body::errors::ExtractBufferedBodyError::into_response(&v4);
                    <pavex::Response as pavex::IntoResponse>::into_response(v5)
                };
            }
        };
        let v5 = pavex::request::body::UrlEncodedBody::extract(v1, &v4);
        let v6 = match v5 {
            Ok(ok) => ok,
            Err(v6) => {
                return {
                    let v7 =
                        pavex::request::body::errors::ExtractUrlEncodedBodyError::into_response(
                            &v6,
                        );
        ...
        let v7 = urlencoded::search::search_form(&v6);
        <http::StatusCode as pavex::IntoResponse>::into_response(v7)
    }
    struct Next0<'a, T>
    where
        T: std::future::Future<Output = pavex::Response>,
    {
        s_0: pavex::request::body::RawIncomingBody,
        s_1: &'a pavex::request::RequestHead,
        next: fn(pavex::request::body::RawIncomingBody, &'a pavex::request::RequestHead) -> T,
    }
    impl<'a, T> std::future::IntoFuture for Next0<'a, T>
    where
        T: std::future::Future<Output = pavex::Response>,
