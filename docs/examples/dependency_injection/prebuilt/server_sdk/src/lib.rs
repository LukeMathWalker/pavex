...
pub struct ApplicationState {
    pub db_connection_pool: di_prebuilt::pool::DbConnectionPool,
}
impl ApplicationState {
    ...
    async fn _new(v0: di_prebuilt::pool::DbConnectionPool) -> crate::ApplicationState {
        crate::ApplicationState {
            db_connection_pool: v0,
        }
    }
}
    ...
    pub async fn route(
        ...
        match matched_route.value {
            0u32 => match &request_head.method {
                &pavex::http::Method::GET => route_1::entrypoint(&state.db_connection_pool).await,
                _ => {
                    let allowed_methods: pavex::router::AllowedMethods =
...
pub mod route_1 {
    pub async fn entrypoint<'a>(s_0: &'a di_prebuilt::pool::DbConnectionPool) -> pavex::Response {
        let response = wrapping_0(s_0).await;
        response
    }
    async fn stage_1<'a>(s_0: &'a di_prebuilt::pool::DbConnectionPool) -> pavex::Response {
        let response = handler(s_0).await;
        response
    }
    async fn wrapping_0(v0: &di_prebuilt::pool::DbConnectionPool) -> pavex::Response {
    ...
    async fn handler(v0: &di_prebuilt::pool::DbConnectionPool) -> pavex::Response {
        let v1 = di_prebuilt::routes::get_index(v0);
    ...
    where
        T: std::future::Future<Output = pavex::Response>,
    {
        s_0: &'a di_prebuilt::pool::DbConnectionPool,
        next: fn(&'a di_prebuilt::pool::DbConnectionPool) -> T,
    }
    impl<'a, T> std::future::IntoFuture for Next0<'a, T>
    where
        T: std::future::Future<Output = pavex::Response>,
