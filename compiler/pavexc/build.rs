...
pub fn main() -> Result<()> {
    // Compute checksum of rustdoc_processor and its local dependencies.
    ...
    let cache_crate_path = base_path.join("../rustdoc/rustdoc_processor");

    // Find all local crates that rustdoc_processor depends on (transitively)
