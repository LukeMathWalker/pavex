...
use crate::language::{Callable, GenericArgument, PathKind, RawIdentifiers, Type, parse_fq_path};
...
pub(crate) fn resolve_type_path(raw_path: &str, krate_collection: &CrateCollection) -> Type {
    ...
    let path = parse_fq_path(
        &identifiers,
        krate_collection.package_graph(),
        PathKind::Type,
    )
    .unwrap();
    super::resolvers::resolve_type_path(&path, krate_collection).unwrap()
}

/// Resolve a callable path assuming that the crate is a dependency of `pavex`.
pub(crate) fn resolve_framework_callable_path(
    raw_path: &str,
    ...
    let path = parse_fq_path(&identifiers, package_graph, PathKind::Callable).unwrap();
