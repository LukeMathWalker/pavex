        ...
        match matched_route.value {
            0u32 => {
                match &request_head.method {
                    &pavex::http::Method::GET => route_1::entrypoint(&state.a).await,
                    _ => {
                        let allowed_methods: pavex::router::AllowedMethods = pavex::router::MethodAllowList::from_iter([
...
pub mod route_1 {
    pub async fn entrypoint<'a>(s_0: &'a app_028e5d85::A) -> pavex::Response {
        let response = wrapping_0(s_0).await;
        response
    }
    async fn stage_1<'a>(s_0: &'a app_028e5d85::A) -> pavex::Response {
        let response = handler(s_0).await;
        response
    }
    async fn wrapping_0(v0: &app_028e5d85::A) -> pavex::Response {
    ...
    async fn handler(v0: &app_028e5d85::A) -> pavex::Response {
        let v1 = app_028e5d85::b(v0);
        let v2 = match v1 {
            Ok(ok) => ok,
            Err(v2) => {
                return {
                    let v3 = app_028e5d85::error_handler(v0, &v2);
                    let v4 = pavex::Error::new(v2);
                    app_028e5d85::error_observer(v0, &v4);
                    <pavex::Response as pavex::IntoResponse>::into_response(v3)
                };
            }
        };
        let v3 = app_028e5d85::handler(&v2);
        <pavex::Response as pavex::IntoResponse>::into_response(v3)
    }
    struct Next0<'a, T>
    where
        T: std::future::Future<Output = pavex::Response>,
    {
        s_0: &'a app_028e5d85::A,
        next: fn(&'a app_028e5d85::A) -> T,
    }
    impl<'a, T> std::future::IntoFuture for Next0<'a, T>
    where
        T: std::future::Future<Output = pavex::Response>,
