...
pub mod route_1 {
    pub async fn entrypoint<'a>(s_0: &'a app_ddc3d7f1::A) -> pavex::Response {
        let response = wrapping_0(s_0).await;
        response
    }
    async fn stage_1<'a>(s_0: &'a app_ddc3d7f1::A) -> pavex::Response {
        let response = handler(s_0).await;
        response
    }
    async fn wrapping_0(v0: &app_ddc3d7f1::A) -> pavex::Response {
    ...
    async fn handler(v0: &app_ddc3d7f1::A) -> pavex::Response {
        let v1 = app_ddc3d7f1::Generic::new(v0);
        let v2 = app_ddc3d7f1::handler(v1);
        <http::StatusCode as pavex::IntoResponse>::into_response(v2)
    }
    struct Next0<'a, T>
    where
        T: std::future::Future<Output = pavex::Response>,
    {
        s_0: &'a app_ddc3d7f1::A,
        next: fn(&'a app_ddc3d7f1::A) -> T,
    }
    impl<'a, T> std::future::IntoFuture for Next0<'a, T>
    where
        T: std::future::Future<Output = pavex::Response>,
