...
pub struct ApplicationState {
    pub http_client: app_f10df8fa::HttpClient,
}
impl ApplicationState {
    pub async fn new(
        _app_config: crate::ApplicationConfig,
        v0: app_f10df8fa::Config,
    ) -> Result<crate::ApplicationState, crate::ApplicationStateError> {
        Ok(Self::_new(v0).await)
    }
    async fn _new(v0: app_f10df8fa::Config) -> crate::ApplicationState {
        let v1 = app_f10df8fa::http_client(v0);
            ...
            0u32 => {
                match &request_head.method {
                    &pavex::http::Method::GET => {
                        route_1::entrypoint(state.http_client.clone(), request_head)
                            .await
                    }
                    _ => {
                        let allowed_methods: pavex::router::AllowedMethods = pavex::router::MethodAllowList::from_iter([
...
pub mod route_1 {
    ...
    async fn stage_1(
        ...
        s_1: app_f10df8fa::HttpClient,
    ) -> pavex::Response {
        let response = handler(s_0, s_1).await;
        response
    }
    async fn wrapping_0(
        v0: app_f10df8fa::HttpClient,
    ...
    async fn handler(
        ...
        v1: app_f10df8fa::HttpClient,
    ) -> pavex::Response {
        let v2 = app_f10df8fa::extract_path(v0).await;
        let v3 = match v2 {
            Ok(ok) => ok,
            Err(v3) => {
                return {
                    let v4 = app_f10df8fa::logger();
                    let v5 = app_f10df8fa::handle_extract_path_error(&v3, v4);
                    <pavex::Response as pavex::IntoResponse>::into_response(v5)
                };
            }
        };
        let v4 = app_f10df8fa::logger();
        let v5 = app_f10df8fa::stream_file(v3, v4, v1);
        <pavex::Response as pavex::IntoResponse>::into_response(v5)
    }
    struct Next0<T>
    where
        T: std::future::Future<Output = pavex::Response>,
    {
        s_0: pavex::request::RequestHead,
        s_1: app_f10df8fa::HttpClient,
        next: fn(pavex::request::RequestHead, app_f10df8fa::HttpClient) -> T,
    }
