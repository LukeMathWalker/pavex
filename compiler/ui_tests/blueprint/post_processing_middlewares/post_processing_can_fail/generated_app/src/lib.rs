...
pub mod route_0 {
    ...
    async fn post_processing_0(v0: pavex::Response) -> pavex::Response {
        let v1 = app_e0af8622::via_attribute(v0);
        let v2 = match v1 {
            Ok(ok) => ok,
            Err(v2) => {
                return {
                    let v3 = app_e0af8622::CustomError::into_response(&v2);
                    <pavex::Response as pavex::IntoResponse>::into_response(v3)
                };
            }
        };
    ...
    async fn post_processing_1(v0: pavex::Response) -> pavex::Response {
        let v1 = app_e0af8622::override_in_blueprint(v0);
        let v2 = match v1 {
            Ok(ok) => ok,
            Err(v2) => {
                return {
                    let v3 = app_e0af8622::CustomError::into_response_override(&v2);
                    <pavex::Response as pavex::IntoResponse>::into_response(v3)
                };
            }
        };
