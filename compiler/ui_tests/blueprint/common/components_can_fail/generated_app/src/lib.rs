...
pub struct ApplicationState {
    pub singleton: app_e501823b::Singleton,
}
...
pub enum ApplicationStateError {
    #[error(transparent)]
    Singleton(app_e501823b::SingletonError),
}
pub fn run(
...
pub mod route_1 {
    pub async fn entrypoint<'a>(s_0: &'a app_e501823b::Singleton) -> pavex::Response {
        let response = wrapping_0(s_0).await;
        response
    }
    async fn stage_1<'a>(s_0: &'a app_e501823b::Singleton) -> pavex::Response {
        let response = handler(s_0).await;
        response
    }
    async fn wrapping_0(v0: &app_e501823b::Singleton) -> pavex::Response {
    ...
    async fn handler(v0: &app_e501823b::Singleton) -> pavex::Response {
        let v1 = app_e501823b::b();
        let v2 = match v1 {
            Ok(ok) => ok,
            Err(v2) => {
                return {
                    let v3 = app_e501823b::ErrorB::into_response(&v2);
                    <pavex::Response as pavex::IntoResponse>::into_response(v3)
                };
            }
        };
        let v3 = app_e501823b::handler(&v2, v0);
        let v4 = match v3 {
            Ok(ok) => ok,
            Err(v4) => {
                return {
                    let v5 = app_e501823b::CustomError::into_response(&v4);
    ...
    where
        T: std::future::Future<Output = pavex::Response>,
    {
        s_0: &'a app_e501823b::Singleton,
        next: fn(&'a app_e501823b::Singleton) -> T,
    }
    impl<'a, T> std::future::IntoFuture for Next0<'a, T>
    where
        T: std::future::Future<Output = pavex::Response>,
