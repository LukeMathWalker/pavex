...
pub mod route_1 {
    pub async fn entrypoint<'a>(s_0: &'a dep_daa9931d::A) -> pavex::Response {
        let response = wrapping_0(s_0).await;
        response
    }
    async fn stage_1<'a>(s_0: &'a dep_daa9931d::A) -> pavex::Response {
        let response = handler(s_0).await;
        response
    }
    async fn wrapping_0(v0: &dep_daa9931d::A) -> pavex::Response {
    ...
    async fn handler(v0: &dep_daa9931d::A) -> pavex::Response {
        let v1 = app_b58d3639::handler(v0);
        <pavex::Response as pavex::IntoResponse>::into_response(v1)
    }
    struct Next0<'a, T>
    where
