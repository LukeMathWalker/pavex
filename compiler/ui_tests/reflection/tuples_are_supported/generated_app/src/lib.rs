            ...
            0u32 => {
                match &request_head.method {
                    &pavex::http::Method::GET => {
                        route_1::entrypoint(state.usize__isize_.clone()).await
                    }
                    _ => {
                        let allowed_methods: pavex::router::AllowedMethods = pavex::router::MethodAllowList::from_iter([
                                pavex::http::Method::GET,
...
pub mod route_1 {
    pub async fn entrypoint(s_0: (usize, isize)) -> pavex::Response {
        let response = wrapping_0(s_0).await;
        response
    }
    async fn stage_1(s_0: (usize, isize)) -> pavex::Response {
        let response = handler(s_0).await;
        response
    }
    async fn wrapping_0(v0: (usize, isize)) -> pavex::Response {
    ...
    async fn handler(v0: (usize, isize)) -> pavex::Response {
        let v1 = app_1bd0738c::handler(v0);
        <pavex::Response as pavex::IntoResponse>::into_response(v1)
    }
    struct Next0<T>
    where
