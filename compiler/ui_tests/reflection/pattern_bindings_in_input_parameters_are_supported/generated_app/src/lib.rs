...
pub struct ApplicationState {
    pub streamer: app_d1b695a6::Streamer,
}
            ...
            0u32 => {
                match &request_head.method {
                    &pavex::http::Method::GET => {
                        route_1::entrypoint(&state.streamer).await
                    }
                    _ => {
                        let allowed_methods: pavex::router::AllowedMethods = pavex::router::MethodAllowList::from_iter([
                                pavex::http::Method::GET,
...
pub mod route_1 {
    pub async fn entrypoint<'a>(s_0: &'a app_d1b695a6::Streamer) -> pavex::Response {
        let response = wrapping_0(s_0).await;
        response
    }
    async fn stage_1<'a>(s_0: &'a app_d1b695a6::Streamer) -> pavex::Response {
        let response = handler(s_0).await;
        response
    }
    async fn wrapping_0(v0: &app_d1b695a6::Streamer) -> pavex::Response {
    ...
    async fn handler(v0: &app_d1b695a6::Streamer) -> pavex::Response {
        let v1 = app_d1b695a6::route_handler(v0);
        <pavex::Response as pavex::IntoResponse>::into_response(v1)
    }
    struct Next0<'a, T>
    where
        T: std::future::Future<Output = pavex::Response>,
    {
        s_0: &'a app_d1b695a6::Streamer,
        next: fn(&'a app_d1b695a6::Streamer) -> T,
    }
    impl<'a, T> std::future::IntoFuture for Next0<'a, T>
    where
        T: std::future::Future<Output = pavex::Response>,
